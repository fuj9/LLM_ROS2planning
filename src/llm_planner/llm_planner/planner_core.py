...
CUTOFF_THRESHOLD = 0.3 # early stopping threshold based on matching score and likelihood score
...
def init_module():
    global generator, translation_lm
    global action_list, action_list_embedding
    global available_examples, example_task_embedding

    generator = lm_engine(source, planning_lm_id, device="cpu")
    translation_lm = SentenceTransformer(translation_lm_id).to(device)

    actions_path = os.path.join(DATA_DIR, "available_robotics_actions.json")
    with open(actions_path, 'r', encoding="utf-8") as f:
        action_list = json.load(f)
    action_list_embedding = translation_lm.encode(
        action_list, batch_size=512, convert_to_tensor=True, device=device
    )

    # examples.json のロード
    examples_path = os.path.join(DATA_DIR, "available_robotics_examples.json")
    ...
    example_task_embedding = translation_lm.encode(
        example_task_list, batch_size=512, convert_to_tensor=True, device=device
    )
